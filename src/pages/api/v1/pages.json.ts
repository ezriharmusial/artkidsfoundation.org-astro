...
export const get: APIRoute = async ({ params, request }) => {
  ...
  const newsList = await getCollection('linkcasts').then((result) => result.map(entry => ({
      author: entry.slug, slug: entry.slug, title: entry.data.title , subtitle: entry.data.subtitle, image: entry.data.image, imageAlt: entry.data.imageAlt
  })));
